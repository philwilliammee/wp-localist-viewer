 ...
 * @param {object} event A localist event obj
...
export const getCalStartDate = event => {
  ...
  return moment(event.date, "MMMM D, YYYY").format("YYYYMMDD");
  //return cd;
};
...
export const getCalEndDate = event => {
  ...
  // const ed = getEventStart(event);
  // const sd = stripDate(ed);
  return getCalStartDate(event);
  //return sd;
};
 ...
 *
 * @param {object} event The localist event.
...
export const getTruncDesc = (event, excerptLength) => {
  ...
  if (excerptLength) {
    description = truncate(description, parseInt(excerptLength, 10));
  }
  return description;
};
 ...
 * @param {object} event The event object.
 *
 * @return {string} The department id.
 */
 ...
 * @return {any} An array or a string if only one.
 */
export const getEventType = (event, prefCategory) => {
  ...
  if (prefCategory === "dept" && parseInt(department, 10) !== 0) {
 ...
 * @todo add support for multiple filter types.
 * @param {object} event The localist Event.
 * @return {any} A string of event types or null
 */
